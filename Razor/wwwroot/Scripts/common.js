...
function load_page (endpoint, container, ...parameters) {
	fetch (endpoint, {
		method: "post",
		headers: { "content-type": "application/json" },
		body: JSON.stringify (parameters.length > 1 ? parameters : parameters [0])
	}).then (result => result.text ()).then (result => document.getElementById (container).innerHTML = result);
}// load_page;

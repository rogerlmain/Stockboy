...
namespace Stockboy.Controllers {
	...
	public class Stocks (DataContext context, StockAPIClient client): Controller {
		...
		public async Task<IActionResult> GetHoldings () {
			try {
				DividendsHandler dividends = new DividendsHandler (context, client);
				HoldingsModelList? price_list = (await HoldingsData.Create (context, client)).HoldingsPriceList ();

				if (is_null (price_list)) return new JsonResult (new { data = no_data });

				return new JsonResult (new HomeModel () {
					payments_list = await dividends.GetPendingPayments (),

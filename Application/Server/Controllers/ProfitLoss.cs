...
namespace Stockboy.Controllers {
	...
	public class ProfitLoss (DataContext context, StockAPIClient client): Controller {
		...
		public async Task<IActionResult> GetProfitAndLoss ([FromBody] UserCredentialsRecord user) {
			try {
				ProfitLossModelList? profit_loss_list = (await HoldingsData.Create (context, client)).GetProfitLossList (user.user_id);
